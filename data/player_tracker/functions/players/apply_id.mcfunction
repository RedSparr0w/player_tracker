...
#execute in minecraft:overworld run data merge block 0 -62 1 {Text1:'{"selector":"@a[tag=player_head,limit=1]"}'}
...
loot replace block 0 -63 1 container.0 loot player_tracker:player_head
data modify block 0 -63 1 Items[0].tag.gui_item set value true
execute store result block 0 -63 1 Items[0].Slot byte 1 run scoreboard players get #current_slot pt.gui
data modify storage pt:player_heads Items append from block 0 -63 1 Items[0]
